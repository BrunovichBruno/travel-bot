...
print("[bot] === BOT VERSION 11 ЗАГРУЖЕНА (multi-layer dedup) ===")
...
def check_against_telegram(url, tg_history):
    """Проверяет, есть ли URL (в любом виде) в истории Telegram."""
    if not url:
        return False
...
def publish_wiki_article(posted, tg_history):
    ...
    if is_posted(source_url, posted):
        print("[wiki] Дубликат в posted.json: " + wiki_data["title"])
        return False

    if check_against_telegram(source_url, tg_history):
        print("[wiki] Дубликат в Telegram: " + wiki_data["title"])
...
def main():
    ...
    # Загружаем posted.json
    posted = load_posted()
    print("[bot] Записей в posted.json: " + str(len(posted)))

    # Загружаем историю Telegram
    ...
    print("[bot] Ссылок в истории Telegram: " + str(len(tg_history)))
